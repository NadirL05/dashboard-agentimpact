...
const PRODUCTS: Product[] = [
  {
    ...
    seoScore: 94,
    seoNote: "Refonte design (fond clair, agent-readiness, JSON-LD inline) + 3 articles /ressources publiés le 20/08. Auth = Supabase (Clerk non migré, risque jugé trop élevé sur une app avec clients réels). CTA tarifs corrigé le 21/08 : renvoie vers /signup plutôt que le Payment Link direct. Audit SEO/GEO technique passé le 21/08 : FAQ JSON-LD dédupliqué /pricing, Organization enrichi (logo/sameAs), Content-Signal corrigé sur les 4 groupes robots.txt, lastModified posé. Score 94/100 annoncé mais pas encore corroboré par un run PSI réel (API rate-limitée pendant l'audit).",
    todo: [
      "Créer un premier témoignage client réel (gap n°1 identifié vs concurrents)",
      "Corroborer le score 94/100 par un vrai run PageSpeed Insights daté (pas encore fait)",
      "Webhook Stripe live câblé sur Vercel (restauyacine) le 21/08 : STRIPE_SECRET_KEY/WEBHOOK_SECRET live + 3 price IDs (Simple/Medium/Luxe). Reste à construire : aucune UI dashboard n'appelle encore /api/stripe/checkout — le paiement 3-tiers n'est pas encore sélectionnable depuis l'app.",
    ],
  },
  {
    name: "PLU IA",
    ...
    seoStatus: "good",
    seoLabel: "Clerk live",
    seoNote: "Identité \"plan cadastral\" + 3D wireframe massing. Auth Clerk basculée en clés live (app-plu-ia.agentimpact.fr). Outil (sas-plu-3d) et landing bien séparés sur leurs sous-domaines respectifs. CTA plan Pro corrigé le 21/08 : renvoie vers le dashboard (inscription) au lieu du Payment Link direct. Audit SEO/GEO passé le 21/08 : rien de CRITICAL, image OG/Twitter ajoutée (absente jusqu'ici), lastModified posé sur le sitemap.",
    todo: [
      "Confirmer via check headless (pas WebFetch) que app-plu-ia.agentimpact.fr (Clerk) est bien en noindex sur ses routes authentifiées",
  ...
  },
  {
    ...
    calendlyNote: 'CTA principal "Voir les tarifs" (paiement direct, renommé le 21/08 — l\'ancien "Commencer gratuitement" était une allégation trompeuse, aucune offre gratuite n\'existe) — Calendly reste en option secondaire.',
    seoStatus: "good",
    seoLabel: "Clerk live",
    seoNote: "Auth Clerk basculée en clés live (app-hector.agentimpact.fr) le 21/08 — CSP corrigée. Audit SEO/GEO passé le 21/08 : 2 CRITICAL trouvés et corrigés (pages légales qui contredisaient le statut Stripe live, CTA \"gratuit\" trompeur) + claims hébergement UE/cloisonnement passés au conditionnel (produit pas encore construit, cf. décision utilisateur).",
    todo: [
      "Décider du serveur EU pour l'hébergement réel de l'outil (piste évoquée : Hostinger + Dokploy, mais serveur actuel jugé trop petit) — la landing dit \"prévu\" en attendant",
      "Contradiction robots.txt non tranchée : Content-Signal ai-train=no déclaré mais Google-Extended reste en Allow (autorise justement l'entraînement Gemini/Bard) — choisir Disallow ou retirer la déclaration ai-train=no",
